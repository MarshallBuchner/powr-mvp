...
const STAGES = [
  ...
  {
    label: "Analyzing skating",
    detail: "Reviewing movement patterns in your footage",
  },
  {
...
export default function AnalysisScreen({
  ...
  const completedSteps = useMemo(
    () => (isComplete ? STAGES.length : activeStep),
    [activeStep, isComplete],
  );

  useEffect(() => {
    // Analysis already finished in UploadCard — keep a short, clear handoff.
    const totalDuration = 2800;

    const stepTimers = STAGES.slice(1).map((_, index) =>
      window.setTimeout(
        () => setActiveStep(index + 1),
        ((index + 1) * totalDuration) / STAGES.length,
      ),
    );
    ...
    <main className={`analysis-page ${isLeaving ? "analysis-leaving" : ""}`}>
      <section className="analysis-shell">
        <div className="analysis-heading">
          ...
          {!isComplete ? (
            <p className="analysis-keep-open">
        ...
        {request.videoUrl ? (
          <div className="analysis-video">
            ...
            <div className="analysis-video-status">
              ...
              <span>
                {isComplete ? "Analysis complete" : STAGES[activeStep].label}
              </span>
            </div>
          </div>
        ) : null}
        ...
        <div className="analysis-card">
                  ...
                  <>
                    ...
                    <span>
                      {Number.isFinite(request.duration)
                        ? `${Math.floor(request.duration / 60)}:${Math.floor(
                            request.duration % 60,
                          )
                            .toString()
                            .padStart(2, "0")}`
          ...
          <div className="progress-section">
            <div className="progress-heading">
              ...
              <strong>{isComplete ? "Done" : "In progress"}</strong>
            </div>

            <div
              className={`progress-track is-indeterminate${
                isComplete ? " is-complete" : ""
              }`}
              role="progressbar"
              aria-label="Analysis progress"
              aria-valuetext={isComplete ? "Complete" : "In progress"}
            >
          ...
          <div className="analysis-quality" aria-label="Assessment quality">
            <p className="analysis-quality-label">Assessment quality</p>
            <p>
              POWR reviews sampled frames from your clip to estimate development
              opportunities. This is coaching feedback — not a scouting grade.
            </p>
          </div>
